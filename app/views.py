...
def deletewish(userid,itemid):
    ...
    response = jsonify({"error":"null","message":"Success. Your wish has been deleted."})
    return response
...
def send_email():
    ...
    try:
        ...
        server.quit()
        response = jsonify({"error":"null", "message":"Your wish has been shared!"})
    except:
        response = jsonify({"error":"1", "message":"Unable to share your wish at the moment. Please try again later."})
    return response
